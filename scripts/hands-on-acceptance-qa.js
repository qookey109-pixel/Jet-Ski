...
async function waitForGame(page, url) {
  ...
  await page.waitForFunction(() => window.JETSKI_RELEASE && window.JETSKI_RACE_MANAGER, null, { timeout: 15000 });
}
...
async function run() {
  ...
  const receipt = {
    ...
    desktop: null,
    mobile: null,
    mobileCaptureCollapsedNoThrottleOverlap: false,
    screenshots: []
  };
  ...
  try {
    ...
    receipt.mobileCaptureCollapsedNoThrottleOverlap = true;
