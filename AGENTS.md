...
- V0.7.1 只升級畫面，不改 `src/main.js`、`src/hydrodynamics.js` 或 RealSeaState contract。
- 重力基準為 9.81 m/s²。
...
- 水面幾何高度與 gameplay sampling 必須共用相同 ocean model；視覺 shader 可加入不改變碰撞面的微細節。
...
- 物理與資料調校值集中在 `src/config.js`；純 rendering overlay 應保持可移除，不綁死物理核心。
