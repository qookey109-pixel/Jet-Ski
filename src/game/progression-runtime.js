...
(function (root) {
  ...
  root.addEventListener('jetski:race-finished', event => {
    ...
    if (result.unlockedId) {
      const unlocked = Race.getCourseDefinition(result.unlockedId);
      messages.push(`UNLOCKED · ${unlocked.name}`);
    }
    if (messages.length) {
      setTimeout(() => {
        const results = document.querySelector('[data-jr-screen="results"] .jr-card');
        if (!results) return;
  ...
  root.JETSKI_PROGRESSION = {
    ...
    render,
    reset(){ progress=Core.resetProgress(defs); persist(); Manager.setCourseDefinition(defs[0].id); render(); },
    localOnly: true,
