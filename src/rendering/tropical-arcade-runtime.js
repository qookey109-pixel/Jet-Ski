...
(function (root) {
  ...
  function findBoostButton() {
    ...
    for (const button of buttons) {
      const semantic = `${button.getAttribute('aria-label') || ''} ${button.textContent || ''}`;
      if (/(?:boost|nitro|瞬間加速)/i.test(semantic)) return button;
    }
    // Language-neutral fallback for the existing V0.11.2 fixed Boost control. This only
    // identifies the visual surface; no event handlers or Boost state are changed.
    for (const button of buttons) {
      const style = button.style;
      if (style && style.position === 'fixed' && style.zIndex === '29' && style.bottom === '142px') return button;
    }
    return null;
  }

  function attachBoostSkin() {
    const boost = findBoostButton();
    if (!boost) return false;
    boost.classList.add('v01116-arcade-boost');
    state.boostSkinAttached = true;
    return true;
  }

  function installArcadeSkin() {
    document.body.classList.add('v01116-tropical-arcade');
    if (!document.querySelector('#v01116-tropical-arcade-style')) {
